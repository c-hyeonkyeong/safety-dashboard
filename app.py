...
def sanitize_config_df(df):
    """설정 데이터프레임의 옵션을 강제로 정리하는 함수"""
    target_cols = ['특별교육과목1', '특별교육과목2']
    for col in target_cols:
        if col not in df.columns:
            df[col] = "해당없음"
    
    # 기존에 다른 값이 들어있어도 강제로 옵션 내의 값으로 변환
    for col in target_cols:
...
# [자동 보정] 실행될 때마다 설정 파일이 요청 사항(4번, 35번)에 맞는지 강제 확인
            ...
            try:
                ...
                if st.button("부서 등록 실행", type="primary"):
                    if '부서명' not in df_dept_new.columns:
                        ...
                        col_rename_map = {
                            '특별교육 1': '특별교육과목1', '특별교육1': '특별교육과목1',
                            '특별교육 2': '특별교육과목2', '특별교육2': '특별교육과목2'
                        }
...
# [자동 보정] 기존 세션에 '특수검진_대상' 컬럼이 없으면 강제로 추가
...
# [수정 1] 폐기물 담당자 계산 적용 (1095일 = 3년)
# 공백 제거 및 문자열 변환으로 매칭 정확도 향상
...
def calc_next_health(row):
    # [수정 3] 특수검진 대상 체크 해제(False)면 계산 안함
...
with st.sidebar:
    ...
    st.caption("특수검진 대상이 아닌 경우 체크를 해제하세요. (저장 시 반영됨)")
...
with tab2:
    ...
    if not target.empty:
        ...
        cols_config = {
            ...
            "최근_직무교육일": st.column_config.DateColumn("최근 직무교육일"),
            "다음_직무교육일": st.column_config.DateColumn("다음 예정일 (3년)", disabled=True),
            "상태": st.column_config.TextColumn("상태", width="small", disabled=True)
        }
